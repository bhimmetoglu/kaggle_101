...
for (ifold in 1:nActualFolds){
  ...
  # KNN
  #predMod4 <- trainKNN(inTrain, inPred, paramknn)
  ...
  #Xoos[ind.start:ind.end, 5] <- predMod4 # Save predictions from KNN
...
colnames(trainOOS) <- c("Id", "predXGB", "predSVR", "predNNET")#, "predKNN")
